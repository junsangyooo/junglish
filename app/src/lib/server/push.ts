...
export function deleteSubscription(dbs: Dbs, endpoint: string): void {
...
/**
 * Who should hear from us this minute: reminder on, this is their time, they have not
 * finished today, and we have not already sent today. The last check is what keeps a
 * one-minute timer from sending the same reminder sixty times.
 */
export function dueReminders(dbs: Dbs, now = new Date()): Reminder[] {
	const rows = dbs.progress.prepare(
		`select id, name, current_day from users
		 where notify_enabled=1 and notify_at=? and (notify_last_sent is null or notify_last_sent <> ?)`
	).all(seoulClock(now), localDate(now)) as { id: number; name: string; current_day: number }[];
...
export async function sendReminder(dbs: Dbs, reminder: Reminder): Promise<number> {
	...
	for (const s of subs) {
		try {
			...
			if (status === 404 || status === 410) deleteSubscription(dbs, s.endpoint);
		}
	}
	return sent;
}
